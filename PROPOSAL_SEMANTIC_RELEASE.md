...
- **Phase 1 (Approx. [Specify Timeframe, e.g., 2-4 Weeks]):**
    - Announce the semantic release proposal and adoption plan to all engineering teams.
    - Begin applying `semantic-release` to all *new* repositories created.
    - Identify 1-2 pilot teams/projects to adopt `semantic-release` on existing repositories. Provide hands-on support and gather feedback.
- **Phase 2 (Approx. [Specify Timeframe, e.g., Next 4-8 Weeks]):**
    - Based on pilot feedback, refine documentation and support materials.
    - Implement commit linting (`commitlint` with `husky`) and PR linting (`amannn/action-semantic-pull-request`) on the pilot repositories and new repositories.
    - Gradually roll out `semantic-release` and associated linting to more teams/repositories.
- **Phase 3 (Ongoing):**
    - Aim for organization-wide adoption of `semantic-release` for all applicable repositories.
    - Continuously monitor the process, gather feedback, and provide ongoing support.
    - Regularly review and update best practices and documentation.

### Migrating Existing Projects

When migrating an existing project to `semantic-release`:

- **Do Not Rewrite History:** Apply Conventional Commit standards and `semantic-release` automation for new pull requests and merges moving forward. Do not attempt to rewrite the existing commit history to conform to Conventional Commits, as this is risky and time-consuming. `semantic-release` will base its first release on commits made after the last existing valid SemVer tag, or from the beginning if no such tags exist (see "Initial Release and Existing Tags" section).
- **Update Merge Strategy:** Ensure the repository's merge strategy is compatible (e.g., Squash Merging, with PR title and description used for the commit message) as outlined in the `PROPOSAL_COMMITS_MERGE.md`.
- **Implement GitHub Actions Workflow:** Add or update the GitHub Actions workflow to include the `semantic-release` job, configured with GitHub App authentication and appropriate triggers (e.g., `workflow_dispatch`).
- **Initial Tagging (Optional but Recommended):** Consider manually creating a SemVer tag that reflects the current "true" version of the project *before* the first `semantic-release` run. This gives `semantic-release` a clear baseline. (Refer to "Initial Release and Existing Tags" for more details).
- **Dry Runs:** Extensively use dry runs (`"dryRun": true` in `.releaserc.json` or `dry_run: true` in the action) to test the setup and ensure `semantic-release` calculates the next version and generates changelog entries as expected before enabling live releases.

### Emergency Bypass Procedures

While `semantic-release` and protected branches aim to create a stable and automated release process, there might be rare, time-sensitive emergencies (e.g., critical production hotfix) requiring a bypass of standard checks.

**Process (General Guidance - Requires Admin Privileges):**

1.  **Approval:** Obtain explicit approval from relevant stakeholders (e.g., tech lead, engineering manager) before initiating a bypass. Document the reason for the bypass.
2.  **Admin Action:** A repository administrator may need to:
    *   Temporarily disable specific branch protection rules (like "Require status checks to pass before merging" or "Require a pull request before merging") on the main branch.
    *   Or, temporarily add the user performing the emergency merge to a bypass list for those rules if using GitHub Rulesets.
3.  **Manual Merge & Push:** The emergency fix can then be merged directly to the main branch, or a PR can be merged without passing checks.
    *   **Crucially, ensure the commit message for this emergency merge *manually* includes `[skip ci]` if it's a version bump or similar automated commit, to prevent re-triggering workflows unnecessarily.**
    *   If the emergency fix itself should trigger a release, ensure its commit message *is* a valid Conventional Commit.
4.  **Post-Bypass Actions:**
    *   **Immediately** re-enable the branch protection rules.
    *   Thoroughly review and test the emergency change.
    *   Manually verify if a new release/tag is needed and if `semantic-release` would have handled it correctly. If not, a manual tag might be required.
    *   Document the bypass event, the fix applied, and any follow-up actions.

**Risks of Bypassing Checks:**

- **Introduction of Bugs/Vulnerabilities:** Skipping CI (tests, linting, security scans) significantly increases the risk of introducing regressions, bugs, or security vulnerabilities into the main branch and subsequent releases.
- **Incorrect Versioning/Changelog:** If Conventional Commit standards are not followed for the bypassed merge, `semantic-release` may calculate an incorrect next version or generate an incomplete/misleading changelog.
- **Breaking Automated Processes:** Bypassing can disrupt the expected flow and reliability of the automated release pipeline.
- **CI/CD Pipeline Instability:** Forgetting `[skip ci]` on manual version commits can lead to CI loops or unexpected workflow triggers.
- **Increased Manual Effort:** A bypass necessitates more manual verification, potential manual tagging, and careful cleanup, negating some of the automation benefits.
- **Erosion of Trust:** Frequent bypassing can erode trust in the automated system and lead to a culture of circumventing established processes.

**Recommendation:** Emergency bypasses should be extremely rare and treated as a last resort. The primary focus should be on robust testing, pre-release validation, and quick rollback strategies to avoid needing such bypasses.
