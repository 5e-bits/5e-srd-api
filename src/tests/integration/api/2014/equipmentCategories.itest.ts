...
describe('/api/2014/equipment-categories/:index', () => {
  it('should return one object', async () => {
    const index = 'adventuring-gear'
    const res = await request(app).get(`/api/2014/equipment-categories/${index}`)
    expect(res.statusCode).toEqual(200)
  ...
  describe('with an invalid index', () => {
    it('should return 404', async () => {
      ...
      const res = await request(app).get(`/api/2014/equipment-categories/${invalidIndex}`)
