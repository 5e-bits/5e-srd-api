...
describe('/api/2024/classes', () => {
  it('should list classes', async () => {
    const res = await request(app).get('/api/2024/classes')
  ...
  describe('with name query', () => {
    it('returns the named object', async () => {
      ...
      const res = await request(app).get(`/api/2024/classes?name=${name}`)
      expect(res.statusCode).toEqual(200)
    ...
    it('is case insensitive', async () => {
      const indexRes = await request(app).get('/api/2024/classes')
      const name = indexRes.body.results[0].name
      const res = await request(app).get(`/api/2024/classes?name=${name.toLowerCase()}`)
      expect(res.statusCode).toEqual(200)
      expect(res.body.results[0].name).toEqual(name)
    })
  })
  ...
  describe('/api/2024/classes/:index', () => {
    it('should return one object', async () => {
      ...
      const showRes = await request(app).get(`/api/2024/classes/${index}`)
    ...
    describe('with an invalid index', () => {
      it('should return 404', async () => {
        const showRes = await request(app).get('/api/2024/classes/invalid-index')
    ...
    describe('/api/2024/classes/:index/levels', () => {
      it('returns objects', async () => {
        const indexRes = await request(app).get('/api/2024/classes')
        const index = indexRes.body.results[0].index
        const res = await request(app).get(`/api/2024/classes/${index}/levels`)
        expect(res.statusCode).toEqual(200)
      ...
      it('includes subclass levels matching the subclass query param', async () => {
        const indexRes = await request(app).get('/api/2024/classes')
        const index = indexRes.body.results[0].index
        const classRes = await request(app).get(`/api/2024/classes/${index}`)
      ...
      describe('/api/2024/classes/:index/levels/:level', () => {
        it('returns objects', async () => {
          ...
          const index = indexRes.body.results[0].index
          const level = 1
          const res = await request(app).get(`/api/2024/classes/${index}/levels/${level}`)
          expect(res.statusCode).toEqual(200)
        ...
        it('returns 400 for an out-of-range level', async () => {
          const indexRes = await request(app).get('/api/2024/classes')
          const index = indexRes.body.results[0].index
          const res = await request(app).get(`/api/2024/classes/${index}/levels/21`)
