...
describe('/api/2014/monsters', () => {
  it('should list monsters', async () => {
    const res = await request(server).get('/api/2014/monsters')
  ...
  it('should hit the cache', async () => {
    ...
    await request(server).get('/api/2014/monsters')
    const res = await request(server).get('/api/2014/monsters')
    expect(res.statusCode).toEqual(200)
    expect(res.body.results.length).not.toEqual(0)
  ...
  describe('with name query', () => {
    it('returns the named object', async () => {
      const indexRes = await request(server).get('/api/2014/monsters')
      const name = indexRes.body.results[1].name
      const res = await request(server).get(`/api/2014/monsters?name=${name}`)
      expect(res.statusCode).toEqual(200)
    ...
    it('is case insensitive', async () => {
      ...
      const res = await request(server).get(`/api/2014/monsters?name=${queryName}`)
      expect(res.statusCode).toEqual(200)
      expect(res.body.results[0].name).toEqual(name)
    })
  })
  ...
  describe('with challenge_rating query', () => {
    describe('with only one provided challenge rating', () => {
      it('returns expected objects', async () => {
        ...
        const res = await request(server).get(`/api/2014/monsters?challenge_rating=${expectedCR}`)
        expect(res.statusCode).toEqual(200)

        const randomIndex = Math.floor(Math.random() * res.body.results.length)
        const randomResult = res.body.results[randomIndex]

        const indexRes = await request(server).get(`/api/2014/monsters/${randomResult.index}`)
    ...
    describe('with many provided challenge ratings', () => {
      it('returns expected objects', async () => {
        const cr1 = 1
        const cr1Res = await request(server).get(`/api/2014/monsters?challenge_rating=${cr1}`)
        expect(cr1Res.statusCode).toEqual(200)

        const cr20 = 20
        const cr20Res = await request(server).get(`/api/2014/monsters?challenge_rating=${cr20}`)
        ...
        const altBothRes = await request(server).get(
  ...
  describe('/api/2014/monsters/:index', () => {
    it('should return one object', async () => {
      ...
      const showRes = await request(server).get(`/api/2014/monsters/${index}`)
    ...
    describe('with an invalid index', () => {
      it('should return 404', async () => {
        ...
        const showRes = await request(server).get(`/api/2014/monsters/${invalidIndex}`)
