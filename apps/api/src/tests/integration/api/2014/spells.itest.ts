...
describe('/api/2014/spells', () => {
  it('should list spells', async () => {
    const res = await request(server).get('/api/2014/spells')
  ...
  it('should hit the cache', async () => {
    ...
    await request(server).get('/api/2014/spells')
    const res = await request(server).get('/api/2014/spells')
    expect(res.statusCode).toEqual(200)
    expect(res.body.results.length).not.toEqual(0)
  ...
  describe('with name query', () => {
    it('returns the named object', async () => {
      const indexRes = await request(server).get('/api/2014/spells')
      const name = indexRes.body.results[1].name
      const res = await request(server).get(`/api/2014/spells?name=${name}`)
      expect(res.statusCode).toEqual(200)
    ...
    it('is case insensitive', async () => {
      ...
      const res = await request(server).get(`/api/2014/spells?name=${queryName}`)
      expect(res.statusCode).toEqual(200)
      expect(res.body.results[0].name).toEqual(name)
    })
  })
  ...
  describe('with level query', () => {
    it('returns expected objects', async () => {
      ...
      const res = await request(server).get(`/api/2014/spells?level=${expectedLevel}`)
      expect(res.statusCode).toEqual(200)

      const randomIndex = Math.floor(Math.random() * res.body.results.length)
      const randomResult = res.body.results[randomIndex]

      const indexRes = await request(server).get(`/api/2014/spells/${randomResult.index}`)
    ...
    describe('with many provided level', () => {
      it('returns expected objects', async () => {
        ...
        const res1 = await request(server).get(`/api/2014/spells?level=${expectedLevel1}`)
        expect(res1.statusCode).toEqual(200)

        const expectLevel2 = 8
        const res2 = await request(server).get(`/api/2014/spells?level=${expectLevel2}`)
        expect(res2.statusCode).toEqual(200)

        const bothRes = await request(server).get(
  ...
  describe('with school query', () => {
    it('returns expected objects', async () => {
      ...
      const res = await request(server).get(`/api/2014/spells?school=${expectedSchool}`)
      expect(res.statusCode).toEqual(200)
    ...
    describe('with many provided schools', () => {
      it('returns expected objects', async () => {
        ...
        const res1 = await request(server).get(`/api/2014/spells?school=${expectedSchool1}`)
        expect(res1.statusCode).toEqual(200)

        const expectedSchool2 = 'Evocation'
        const res2 = await request(server).get(`/api/2014/spells?school=${expectedSchool2}`)
        expect(res2.statusCode).toEqual(200)
    ...
    it('is case insensitive', async () => {
      ...
      const showRes = await request(server).get(`/api/2014/spells/${index}`)
      const school = showRes.body.school.name
      const querySchool = school.toLowerCase()
      const res = await request(server).get(`/api/2014/spells?school=${querySchool}`)

      const randomIndex = Math.floor(Math.random() * res.body.results.length)
      const randomResult = res.body.results[randomIndex]

      const queryRes = await request(server).get(`/api/2014/spells/${randomResult.index}`)
  ...
  describe('/api/2014/spells/:index', () => {
    ...
    describe('with an invalid index', () => {
      it('should return 404', async () => {
        ...
        const showRes = await request(server).get(`/api/2014/spells/${invalidIndex}`)
